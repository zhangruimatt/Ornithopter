classdef UBEMStepper2d < handle
    properties
        ...
        xref = -20; % x coordinate of point at infinity (as fraction of chord)
        ...
        maxiters = 600; % max iterations for wake panel convergence
        tol = 1e-5;     % wake panel convergence tolerance
        quadtol = 1e-6; % truncation level for quadratic Kutta term
        maxerr = 1e-5;  % max error for Kutta or Neumann error norms
        % Wake modeling
        wakep_free = false;  % use free wake panel? (false: bisect trailing edge)
        ...
        kutta_implicit = true; % Using implicit Kutta condition (false: simple Kutta condition, circulation = 0)
    ...
    methods
        ...
        function [Cp,phi,gamk] = unsteadyStep(self,dalp,dx,dy,dt)
            ...
            % Move the body based on the kinematic arguments
            self.foil.glide(dalp,dx+self.foil.vx*dt,dy+self.foil.vy*dt);
        ...
        % It includes both the influence of all panels and freestream speed
