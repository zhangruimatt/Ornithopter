...
for i=1:nsteps
    ...
    shape = fill(-f.x/f.chord,f.y/f.chord,[.75 .75 .75]);
    hold on;

ip = find(stepper.wake.nu > 0);
in = find(stepper.wake.nu < 0);
p1 = plot(-stepper.wake.x(ip)/f.chord,stepper.wake.y(ip)/f.chord,'.r',...
    'MarkerSize',10);
p2 = plot(-stepper.wake.x(in)/f.chord,stepper.wake.y(in)/f.chord,'.b',...
    'MarkerSize',10);
set(gca,'FontSize',14);
axis('equal');
ff = getframe(gcf);
